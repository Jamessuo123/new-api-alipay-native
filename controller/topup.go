...
func GetTopUpInfo(c *gin.Context) {
	...
	enableAlipayPc := IsAlipayPcTopUpEnabled()
	if enableAlipayPc {
		hasAlipayPc := false
		for _, method := range payMethods {
			if method["type"] == PaymentMethodAlipayPC { hasAlipayPc = true; break }
		}
		if !hasAlipayPc { payMethods = append(payMethods, map[string]string{"name":"支付宝官方支付","type":PaymentMethodAlipayPC,"color":"#1677FF","min_topup":strconv.Itoa(operation_setting.MinTopUp)}) }
	}

	data := gin.H{
		"enable_online_topup":              isEpayTopUpEnabled() || enableAlipayPc,
...
func buildEpayPaymentURL(uri string, params map[string]string) string {
	values := url.Values{}
...
func RequestEpay(c *gin.Context) {
	...
	if req.PaymentMethod == PaymentMethodAlipayPC {
	...
	paymentURL := buildEpayPaymentURL(uri, params)
	c.JSON(http.StatusOK, gin.H{
		"message":          "success",
		"data":             params,
		"url":              uri,
		"cashier_url":      paymentURL,
		...
		"payment_provider": model.PaymentProviderEpay,
		"create_time":      topUp.CreateTime,
		"created_at":       topUp.CreateTime,
		"expire_at":        topUp.CreateTime + 15*60,
	})
}
...
func buildTopUpOrderItem(topUp *model.TopUp) gin.H {
	if topUp == nil {
		return gin.H{}
	}
...
func buildTopUpOrderItems(topUps []*model.TopUp) []gin.H {
	items := make([]gin.H, 0, len(topUps))
	for _, topUp := range topUps {
		items = append(items, buildTopUpOrderItem(topUp))
	}
	return items
}
...
func GetTopUpStatus(c *gin.Context) {
	...
	common.ApiSuccess(c, buildTopUpOrderItem(topUp))
}
...
func GetUserTopUps(c *gin.Context) {
	...
	pageInfo.SetItems(buildTopUpOrderItems(topups))
