...
type AlipayPCRequest struct {
	Amount        int64  `json:"amount"`
	PaymentMethod string `json:"payment_method"`
}

func IsAlipayPcTopUpEnabled() bool { return alipaypc.IsEnabled() }
func RequestAlipayPcTopUp(c *gin.Context) {
	var req AlipayPCRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "参数错误"})
		return
	}
	RequestAlipayPcTopUpFromAmount(c, req.Amount)
}
func RequestAlipayPcTopUpFromAmount(c *gin.Context, requestAmount int64) {
	if !IsAlipayPcTopUpEnabled() {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "支付宝 PC 支付未启用"})
		return
	}
	...
	amount := normalizeAlipayTopUpAmount(requestAmount)
	...
	cashierURL, err := alipaypc.CreatePagePayURL(tradeNo, fmt.Sprintf("FeiXiangApi 充值 %d", requestAmount), payMoney, notifyURL, returnURL)
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("支付宝 PC 拉起支付失败 user_id=%d trade_no=%s amount=%d error=%q", id, tradeNo, requestAmount, err.Error()))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "拉起支付宝支付失败"})
		return
	}
	topUp := &model.TopUp{UserId: id, Amount: amount, Money: payMoney, TradeNo: tradeNo, PaymentMethod: PaymentMethodAlipayPC, PaymentProvider: PaymentProviderAlipayPC, CreateTime: time.Now().Unix(), Status: common.TopUpStatusPending}
	if err := topUp.Insert(); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("支付宝 PC 创建充值订单失败 user_id=%d trade_no=%s amount=%d error=%q", id, tradeNo, requestAmount, err.Error()))
...
func GetAlipayPcTopUpStatus(c *gin.Context) {
	...
	common.ApiSuccess(c, buildAlipayTopUpOrderItem(topUp))
}
func AlipayPcReturn(c *gin.Context) {
	fallback := service.GetCallbackAddress() + "/dashboard/wallet"
	c.Redirect(http.StatusFound, alipaypc.FrontendReturnURL(fallback))
}
func AlipayPcNotify(c *gin.Context) {
	...
	if err := alipaypc.VerifyNotify(params); err != nil {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("支付宝 PC 回调验签失败 client_ip=%s error=%q params=%q", c.ClientIP(), err.Error(), common.GetJsonString(params)))
		_, _ = c.Writer.Write([]byte("failure"))
		return
	}
	...
	if handled, ok := completeQuotaPackageAlipayNotifyIfNeeded(c, tradeNo, params); handled {
		if ok {
			_, _ = c.Writer.Write([]byte("success"))
		} else {
			_, _ = c.Writer.Write([]byte("failure"))
	...
	if topUp.PaymentProvider != PaymentProviderAlipayPC {
		_, _ = c.Writer.Write([]byte("failure"))
		return
	}
	if topUp.PaymentMethod == model.PaymentMethodQuotaPackage {
		_, _ = c.Writer.Write([]byte("failure"))
		return
	}
	if totalAmount, err := strconv.ParseFloat(params["total_amount"], 64); err == nil && totalAmount > 0 {
		if math.Abs(totalAmount-topUp.Money) > 0.01 {
			_, _ = c.Writer.Write([]byte("failure"))
			return
		}
	}
	if topUp.Status == common.TopUpStatusPending {
		...
		model.RecordTopupLog(topUp.UserId, fmt.Sprintf("使用支付宝 PC 支付充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(quotaToAdd), topUp.Money), c.ClientIP(), topUp.PaymentMethod, PaymentProviderAlipayPC)
	}
	_, _ = c.Writer.Write([]byte("success"))
}
...
func buildAlipayTopUpOrderItem(topUp *model.TopUp) gin.H {
	if topUp == nil {
		return gin.H{}
	}
	username := ""
