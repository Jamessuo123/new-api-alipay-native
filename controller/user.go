...
func sanitizeSelfInviteDetail(detail *model.AdminUserInviteDetail, currentUserID int) {
	if detail == nil {
		return
	}

	detail.User.Email = ""
	if detail.Inviter != nil {
		detail.Inviter.Email = ""
	}
	for i := range detail.Invitees {
		detail.Invitees[i].Email = ""
	}

	rewardLogs := make([]model.AdminInviteRewardLogItem, 0, len(detail.RewardLogs))
	for _, item := range detail.RewardLogs {
		if item.ReceiverUserID == currentUserID {
			rewardLogs = append(rewardLogs, item)
		}
	}
	detail.RewardLogs = rewardLogs
}

func GetSelfInviteDetail(c *gin.Context) {
	id := c.GetInt("id")
	if id == 0 {
	...
	detail, err := model.GetAdminUserInviteDetail(id)
	if err != nil {
		common.ApiError(c, err)
		return
	}

	sanitizeSelfInviteDetail(detail, id)
	common.ApiSuccess(c, detail)
}

func parseInviteConsumptionIntParam(c *gin.Context, key string, defaultValue int, minValue int, maxValue int) int {
...
func parseInviteConsumptionTimeRange(c *gin.Context) (int64, int64) {
	...
	if value := strings.TrimSpace(c.Query("start_time")); value != "" {
		if parsed, err := strconv.ParseInt(value, 10, 64); err == nil && parsed > 0 {
			startTimestamp = parsed
		}
	}
	if value := strings.TrimSpace(c.Query("end_time")); value != "" {
		if parsed, err := strconv.ParseInt(value, 10, 64); err == nil && parsed > 0 {
			endTimestamp = parsed
		}
	}
...
func GetSelfInviteConsumption(c *gin.Context) {
	id := c.GetInt("id")
	if id == 0 {
		common.ApiError(c, errors.New("用户未登录"))
		return
	...
	startTimestamp, endTimestamp := parseInviteConsumptionTimeRange(c)
	page := parseInviteConsumptionIntParam(c, "page", 1, 1, 100000)
	pageSize := parseInviteConsumptionIntParam(c, "page_size", 20, 1, 100)
	sortKey := strings.TrimSpace(c.Query("sort"))

	data, err := model.GetInviteConsumptionOverview(model.InviteConsumptionQuery{
		InviterUserID:  id,
...
func GetUserInviteDetail(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil || id == 0 {
		common.ApiError(c, err)
		return
