...
/** * Request official Alipay PC payment */
export async function requestAlipayPcPayment(
  request: PaymentRequest
): Promise<PaymentResponse> {
  const res = await api.post('/api/user/topup/alipay/pc', request, {
    skipBusinessError: true,
  } as Record<string, unknown>)
  return {
    ...res.data,
    url: res.data?.url || (res as unknown as { url?: string }).url || res.data?.cashier_url || res.data?.payment_url,
  }
}
