...
// FEIXIANG_SEEDANCE_PRICING_ORIGINAL_UI_V2_DETAILS
// FEIXIANG_VIDEO_PRICING_COPY_CLARITY_V4_3_DETAILS
type FeiXiangSeedancePricingRow = {
  resolution: string
  withVideoInput: string
  withoutVideoInput: string
}

function getFeiXiangSeedancePricingProfile(modelName?: string):
  | { modelId: string; title: string; rows: FeiXiangSeedancePricingRow[] }
  | null {
  const name = modelName || ''
  if (/^seedance-2\.0-fast-/.test(name)) {
    return {
...
function SeedanceVideoPricingSection(props: {
  model: PricingModel
  tokenUnit: TokenUnit
}) {
  ...
  return (
    <div className='space-y-4'>
      <SectionTitle>定价信息</SectionTitle>
      <div className='overflow-hidden rounded-2xl border bg-card/50'>
        <div className='grid grid-cols-1 divide-y md:grid-cols-[1.25fr_.7fr_1.55fr] md:divide-x md:divide-y-0'>
          <div className='space-y-4 p-5'>
            <div className='text-muted-foreground text-xs font-medium tracking-wider uppercase'>
              服务提供商
            </div>
            <div className='inline-flex max-w-full items-center gap-2 rounded-md border bg-muted/40 px-3 py-2 text-xs'>
          ...
          <div className='space-y-4 p-5'>
            ...
            <span className='inline-flex rounded-md bg-muted px-3 py-2 text-sm font-semibold'>
              按Token计费
            </span>
            <p className='text-muted-foreground text-xs'>
              输出视频已包含在视频生成单价中，不再单独展示输出 Token 价格，避免重复计费。
            </p>
          </div>
          ...
          <div className='space-y-4 p-5'>
            ...
            <div className='grid gap-3 sm:grid-cols-2'>
              {profile.rows.map((row) => (
                <div
                  key={profile.modelId + '-' + row.resolution}
                  className='rounded-xl border bg-background/40 p-3'
                >
                  <div className='mb-2 text-sm font-semibold'>{row.resolution}</div>
                  <div className='grid gap-1.5 text-xs'>
                    <div className='flex items-center justify-between gap-3'>
                      <span className='text-muted-foreground'>含视频输入</span>
                      <span className='font-mono font-semibold text-primary'>
                        {row.withVideoInput} / {tokenUnitLabel}
                      </span>
                    </div>
                    <div className='flex items-center justify-between gap-3'>
                      <span className='text-muted-foreground'>无视频输入</span>
                      <span className='font-mono font-semibold text-primary'>
                        {row.withoutVideoInput} / {tokenUnitLabel}
                      </span>
                    </div>
                  </div>
                </div>
...
function PriceSection(props: {
  ...
  const { t } = useTranslation()
  if (getFeiXiangSeedancePricingProfile(props.model.model_name)) {
    return (
      <SeedanceVideoPricingSection
        model={props.model}
          ...
          <table className='min-w-full border-collapse text-sm'>
            <thead>
              ...
              <tr className='border-b bg-muted/20'>
                ...
                {profile.rows.map((row) => (
                  <>
                    <th key={`${row.resolution}-with-base`} className={thClass}>
                      default 含视频
                    </th>
                    <th key={`${row.resolution}-with-final`} className={thClass}>
                      最终 含视频
                    </th>
                    <th key={`${row.resolution}-without-base`} className={thClass}>
                      default 无视频
                    </th>
                    <th key={`${row.resolution}-without-final`} className={thClass}>
                      最终 无视频
                    </th>
                  </>
                ))}
                    ...
                    {profile.rows.map((row) => {
                      ...
                      return (
                        <>
                          <td
                            key={`${group}-${row.resolution}-with-base`}
                            className={`${tdClass} font-mono text-muted-foreground`}
                          >
                            {formatFeiXiangSeedanceUsdPrice(withVideo)}
                          </td>
                          <td
                            key={`${group}-${row.resolution}-with-final`}
                            className={`${tdClass} font-mono font-semibold text-primary`}
                          >
                            {formatFeiXiangSeedanceUsdPrice(withVideo * ratio)}
                          </td>
                          <td
...
function GroupPricingSection(props: {
  ...
  if (getFeiXiangSeedancePricingProfile(props.model.model_name)) {
    return (
      <FeiXiangSeedanceGroupPricingCompareTable
        model={props.model}
