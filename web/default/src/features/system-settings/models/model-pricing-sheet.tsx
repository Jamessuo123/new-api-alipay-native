...
// FEIXIANG_VIDEO_DUAL_PRICE_V5_SHEET_LOCAL_HELPER
const FEIXIANG_VIDEO_DUAL_PRICE_MODEL_RE_V5 =
  /(seedance|veo|kling|sora|wan|hailou|hailuo|skyreels|video|veo-|kling-)/i

function isFeiXiangVideoInputPricingModelV5(modelName?: string | null): boolean {
  return FEIXIANG_VIDEO_DUAL_PRICE_MODEL_RE_V5.test(String(modelName || ''))
}
...
// FEIXIANG_VIDEO_ADMIN_DUAL_FIELD_LOGIC_V1
// 后台视频模型定价语义：无视频输入价格沿用 ModelRatio；含视频输入价格保存到 VideoInputRatio。
// 文本模型仍保持原有输入 / 输出 / 缓存定价逻辑。

// FEIXIANG_VIDEO_WITH_INPUT_FIELD_MAP_V1
// 视频模型后台定价字段映射：无视频输入价格使用 ModelRatio；含视频输入价格使用 VideoInputRatio。
// 不改变文本模型的输入/输出/缓存/分组倍率逻辑。
const FEIXIANG_VIDEO_WITH_INPUT_MODEL_RE_V1 =
  /(seedance|veo|kling|sora|wan|hailuo|skyreels|video)/i

function isFeiXiangVideoWithInputModelV1(modelName?: string | null): boolean {
  return FEIXIANG_VIDEO_WITH_INPUT_MODEL_RE_V1.test(String(modelName || ''))
}

function getFeiXiangVideoWithInputFormModelNameV1(form: { watch: (name: string) => unknown }): string {
  return String(form.watch('name') || '')
}

function isFeiXiangVideoWithInputFormV1(form: { watch: (name: string) => unknown }): boolean {
  return isFeiXiangVideoWithInputModelV1(getFeiXiangVideoWithInputFormModelNameV1(form))
}
const FEIXIANG_VIDEO_ADMIN_MODEL_RE_V1 =
  /(seedance|veo|kling|sora|wan|hailuo|skyreels|video)/i

function isFeiXiangVideoAdminPricingModelV1(modelName?: string | null): boolean {
  return FEIXIANG_VIDEO_ADMIN_MODEL_RE_V1.test(String(modelName || ''))
}

function getFeiXiangVideoAdminModelNameV1(form: { watch: (name: string) => unknown }): string {
  return String(form.watch('name') || '')
}
...
>(function ModelPricingEditorPanel(
  ...
  const form = useForm<ModelPricingFormValues>({
    ...
    defaultValues: {
      ...
      videoInputRatio: '', // FEIXIANG_VIDEO_DUAL_PRICE_FINAL_DEFAULT
    },
  })
  ...
  useEffect(() => {
    ...
    if (editData) {
      form.reset({
        ...
        completionRatio: editData.completionRatio || '', // FEIXIANG_VIDEO_DUAL_PRICE_V9_1_KEEP_COMPLETION_LOAD
        imageRatio: editData.imageRatio || '',
        audioRatio: editData.audioRatio || '',
        audioCompletionRatio: editData.audioCompletionRatio || '',
        videoInputRatio: editData.videoInputRatio || '', // FEIXIANG_VIDEO_DUAL_PRICE_FINAL_LOAD
      })
      setPricingMode(
      ...
      form.reset({
        ...
        audioRatio: '',
        audioCompletionRatio: '',
        videoInputRatio: '', // FEIXIANG_VIDEO_DUAL_PRICE_FINAL_EMPTY
      })
  ...
  const handleLanePriceChange = (lane: LaneKey, value: string) => {
    ...
    const nextLaneEnabled = { ...laneEnabled }
    const hasNextValue = hasValue(value)

    // FEIXIANG_MODEL_PRICING_VISUAL_DIRTY_SYNC_FIX_V1
    // The compact price lane UI does not render an explicit enable switch.
    // Treat typing a price as enabling that lane, and clearing it as disabling
    // the lane. Otherwise newly added optional prices (completion/cache/image)
    // update the visible input only but never enter the saved ratio JSON.
    nextLaneEnabled[lane] = hasNextValue

    if (lane === 'audioInput' && !hasNextValue) {
      nextLaneEnabled.audioOutput = false
    ...
    if (lane === 'audioInput' && nextLaneEnabled.audioOutput) {
      setFormValue(
        'audioCompletionRatio',
        deriveLaneRatio(
          'audioOutput',
  ...
  const buildSubmitData = useCallback(
    (values: ModelPricingFormValues) => {
      const data: ModelRatioData = {
        ...
        videoInputRatio: values.videoInputRatio || '', // FEIXIANG_VIDEO_DUAL_PRICE_FINAL_COMMIT
      }


      if (isFeiXiangVideoAdminPricingModelV1(data.name)) {
        // FEIXIANG_VIDEO_DUAL_PRICE_V9_1_KEEP_COMPLETION_AND_VIDEO_INPUT
        // 视频模型保留补全价格 CompletionRatio；含视频输入价格独立保存到 VideoInputRatio。
        data.videoInputRatio = values.videoInputRatio || data.videoInputRatio || ''
      }
                ...
                >
                  ...
                  <TabsContent value='per-token' className='pt-0'>
                    <FieldGroup className='gap-5'>
                      <Field>
                        <FieldLabel>{isFeiXiangVideoAdminPricingModelV1(getFeiXiangVideoAdminModelNameV1(form)) ? '无视频输入价格' : t('Input price')}</FieldLabel>
                        <PriceInput
                          value={promptPrice}
                          placeholder='3'
                      ...
                      {isFeiXiangVideoAdminPricingModelV1(
                        form.watch('name') || editData?.name || ''
                      ) ? (
                        <PriceLane
                          modelName={form.watch('name') || editData?.name || ''}
                          lane='videoInput'
                          fieldName='videoInputRatio'
                          form={form}
                          title='含视频输入价格'
                          description='带图片 / 视频输入 token 的美元价格，保存到 VideoInputRatio。'
                          placeholder='6.2'
                          value={form.watch('videoInputRatio') || ''}
                          enabled={true}
                          disabled={false}
                          onChange={(value) => {
                            if (numericDraftRegex.test(value)) {
                              form.setValue('videoInputRatio', value, {
                                shouldDirty: true,
                                shouldValidate: true,
                              })
                            }
