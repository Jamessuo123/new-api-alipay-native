...
// FEIXIANG_VIDEO_DUAL_PRICE_V5_VISUAL_HELPER
const FEIXIANG_VIDEO_DUAL_PRICE_MODEL_RE_V5 =
  /(seedance|veo|kling|sora|wan|hailou|hailuo|skyreels|video|veo-|kling-)/i

function isFeiXiangVideoInputPricingModelV5(
  modelName?: string | null
): boolean {
  return FEIXIANG_VIDEO_DUAL_PRICE_MODEL_RE_V5.test(String(modelName || ''))
}

function getFeiXiangVideoInputPriceStringV5(value: unknown): string {
  if (value === undefined || value === null || value === '') return ''
  const parsed = typeof value === 'number' ? value : Number.parseFloat(String(value))
  return Number.isFinite(parsed) ? String(parsed) : ''
}

// FEIXIANG_VIDEO_INPUT_RATIO_VISUAL_PERSIST_V1
// 视觉编辑器保存链路：视频模型的“含视频输入价格”写入 VideoInputRatio；
// “无视频输入价格”继续写入 ModelRatio；文本模型逻辑不变。
const FEIXIANG_VIDEO_INPUT_RATIO_MODEL_RE_V1 =
  /(seedance|veo|kling|sora|wan|hailuo|skyreels|video)/i

function isFeiXiangVideoInputRatioModelV1(modelName?: string | null): boolean {
  return FEIXIANG_VIDEO_INPUT_RATIO_MODEL_RE_V1.test(String(modelName || ''))
}

function getFeiXiangVideoInputRatioStringV1(value: unknown): string {
  if (value === undefined || value === null || value === '') return ''
  const parsed = typeof value === 'number' ? value : Number.parseFloat(String(value))
  return Number.isFinite(parsed) ? String(parsed) : ''
}
...
>(function ModelRatioVisualEditor(
  ...
  const models = useMemo(() => {
    ...
    const videoInputRatioMapForRows = safeJsonParse<Record<string, number>>(
      videoInputRatio,
    ...
    return Array.from(modelNames)
      .map((name) => {
        ...
        return {
          ...displayed!,
            videoInputRatio:
              getFeiXiangVideoInputPriceStringV5(videoInputRatioMapForRows[name]) ||
              ((displayed as ModelRatioData).videoInputRatio ?? ''), // FEIXIANG_VIDEO_DUAL_PRICE_V5_ROW_VALUE
          saved,
          draft,
  ...
  const handleDelete = useCallback(
    (name: string) => {
      ...
      const videoInputRatioMap = safeJsonParse<Record<string, number>>(
        videoInputRatio,
        { fallback: {}, silent: true }
      )
      ...
      delete videoInputRatioMap[name]
      ...
      onChange('VideoInputRatio', JSON.stringify(videoInputRatioMap, null, 2))
  ...
  const persistPricingData = useCallback(
    (data: ModelRatioData, targetNames: string[] = [data.name]) => {
      ...
      targetNames.forEach((name) => {
        ...
        const isVideoModel = isFeiXiangVideoInputRatioModelV1(name)
        ...
        if (data.billingMode === 'tiered_expr') {
          ...
          if (isVideoModel) {
            setIfPresent(videoInputRatioMap, name, data.videoInputRatio)
          }
...
export const ModelRatioVisualEditor = memo(
  ...
  (prevProps, nextProps) => {
    return (
      ...
      prevProps.savedAudioCompletionRatio === nextProps.savedAudioCompletionRatio &&
      prevProps.savedVideoInputRatio === nextProps.savedVideoInputRatio &&
      ...
      prevProps.videoInputRatio === nextProps.videoInputRatio &&
