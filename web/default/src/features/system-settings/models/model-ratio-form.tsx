...
type ModelFormValues = {
  ...
  VideoInputRatio: string // FEIXIANG_VIDEO_INPUT_RATIO_PRICING_V1_FORM_VALUE
  VideoResolutionPricing: string // FEIXIANG_VIDEO_RESOLUTION_PRICING_V4_1_FORM_VALUE
...
type ModelJsonFieldName =
  ...
  | 'VideoInputRatio' // FEIXIANG_VIDEO_INPUT_RATIO_PRICING_V1_FORM_FIELD
  | 'VideoResolutionPricing' // FEIXIANG_VIDEO_RESOLUTION_PRICING_V4_1_FORM_FIELD
...
const modelJsonFields: Array<{
  ...
  {
    name: 'VideoInputRatio', // FEIXIANG_VIDEO_DUAL_INPUT_PRICING_V1_FORM_FIELD
    labelKey: 'Video input pricing',
    descriptionKey: 'Video models only: JSON map for with-video-input price. No-video-input continues to use ModelRatio.',
    placeholder: '{\n  "seedance-2.0-480p": 3.1\n}',
  },
  {
    name: 'VideoResolutionPricing', // FEIXIANG_VIDEO_RESOLUTION_PRICING_V4_1_JSON_FIELD
    labelKey: '视频分辨率定价',
    descriptionKey:
      '视频基础模型 → 分辨率 → 无视频输入 / 含视频输入 / 补全价格的 JSON 映射。',
  },
]
...
export const ModelRatioForm = memo(function ModelRatioForm({
  ...
  const handleSave = useCallback(async () => {
    if (editMode === 'visual') {
      ...
      // commitOpenEditor writes the editor draft into the parent RHF form via
      // multiple setValue calls. Yield once before handleSubmit so the submit
      // path reads the committed ModelRatio/CompletionRatio/etc. values instead
      // of the previous snapshot and incorrectly reports no changes.
      ...
      <Form {...form}>
        {editMode === 'visual' ? (
          <div className='space-y-6'>
            ...
            {/* FEIXIANG_VIDEO_RESOLUTION_PRICING_V4_1_VISUAL_EDITOR */}
            <div className='rounded-xl border bg-card/50 p-4'>
              <ModelJsonTextareaField
                form={form}
                name='VideoResolutionPricing'
                label={t('视频分辨率定价')}
                description={t('结构：baseModel → resolution → no_video_input / with_video_input / completion。')}
              />
            </div>
