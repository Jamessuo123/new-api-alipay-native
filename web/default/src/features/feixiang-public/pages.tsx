...
func main() {
  config := openai.DefaultConfig("API_KEY_REDACTED")
        ...
        >
          <div className='fx-api-info-grid'>
            ...
            <ApiCard title='认证方式' desc='所有请求通过 Bearer Token 认证。'>
              <code>Authorization: Bearer API_KEY_REDACTED</code>
            </ApiCard>
...
function GiftIcon({ className }: { className?: string }) { return <Svg className={className}><path d='M20 12v10H4V12' /><path d='M2 7h20v5H2z' /><path d='M12 22V7' /><path d='M12 7H7.5a2.5 2.5 0 1 1 0-5C11 2 12 7 12 7Z' /><path d='M12 7h4.5a2.5 2.5 0 1 0 0-5C13 2 12 7 12 7Z' /></Svg> }
...
function LogoMark(props: { rounded?: boolean } = {}) {
  return (
    <img
      src={BRAND_LOGO_SRC}
      alt={`${BRAND} logo`}
      // FEIXIANG_HOME_LOGO_ROUNDED_ONLY_V2: 首页导航 Logo 仅在 rounded=true 时裁切为同尺寸圆角矩形
      data-fx-home-logo-rounded={props.rounded ? 'true' : undefined}
      style={props.rounded ? { borderRadius: '10px' } : undefined}
...
['控制台', '/dashboard'],
    ['模型广场', '/pricing'],
    // FEIXIANG_HOME_AI_CREATION_NAV_ENTRY_FINAL
    ['AI创作', '/creative'],
    ['帮助文档', '/docs'],
  ]
    ...
    <header className='sticky top-0 z-50 border-b border-white/10 bg-[#070718]/88 backdrop-blur-2xl'>
      <div className='mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8'>
        ...
        <div className='hidden items-center gap-3 sm:flex'>
          <FeiXiangSmartLink href='/invite-rewards' className='FEIXIANG_INVITE_REWARDS_HEADER_ENTRY_V1 inline-flex items-center gap-2 rounded-xl border border-cyan-300/35 bg-gradient-to-r from-cyan-400/15 to-violet-500/18 px-4 py-2.5 text-sm font-black text-cyan-50 shadow-[0_14px_34px_rgba(34,211,238,0.14)] transition hover:-translate-y-0.5 hover:border-cyan-200/60 hover:from-cyan-400/28 hover:to-violet-500/30 hover:text-white'><GiftIcon className='h-4 w-4' />邀请奖励</FeiXiangSmartLink>
      ...
      {open && (
        <div className='border-t border-white/10 bg-[#09091d] px-4 py-4 md:hidden'>
          <div className='grid gap-2'>
            ...
            <FeiXiangSmartLink href='/invite-rewards' onClick={() => setOpen(false)} className='FEIXIANG_INVITE_REWARDS_MOBILE_ENTRY_V1 rounded-xl border border-cyan-300/35 bg-gradient-to-r from-cyan-400/15 to-violet-500/20 px-4 py-3 text-center text-sm font-black text-cyan-50'>邀请奖励</FeiXiangSmartLink>
      ...
      <div className='mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 md:grid-cols-[1.4fr_1fr_1fr_1fr] lg:px-8'>
        <div>
          <div className='flex items-center gap-3'>
            {/* FEIXIANG_HOME_FOOTER_LOGO_MATCH_HEADER_INLINE_FINAL */}
            <span
              data-fx-home-footer-logo-match-header-inline-final
              className='inline-flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-[10px] bg-white'
            >
              <img
                src={BRAND_LOGO_SRC}
                alt={`${BRAND} logo`}
            ...
            <strong className='text-lg text-white'>{BRAND}</strong>
          </div>
          <p className='mt-4 max-w-sm text-sm leading-7 text-white/50'>稳定、快速、开放的 AI API 服务平台，让 AI 能力触手可及。</p>
        </div>
        {/* FEIXIANG_HOME_AI_CREATION_FOOTER_ENTRY_FINAL */}
        <FooterLinks title='产品' links={[['AI创作', '/creative'], ['模型广场', '/pricing'], ['控制台', '/dashboard'], ['邀请奖励', '/invite-rewards']]} />
        <FooterLinks title='开发者' links={[['帮助文档', '/docs'], ['快速开始', '/register'], ['API 状态', '/status']]} />
  ...
  return (
    <section className='mx-auto max-w-7xl px-4 pb-12 pt-16 text-center sm:px-6 sm:pt-20 lg:px-8'>
      ...
      <div className='mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row sm:flex-wrap'>
        ...
        {/* FEIXIANG_HOME_AI_CREATION_HERO_ENTRY_FINAL */}
        <FeiXiangSmartLink href='/creative' className='inline-flex min-w-44 items-center justify-center gap-2 rounded-xl border border-fuchsia-300/25 bg-gradient-to-r from-fuchsia-500/18 to-violet-500/18 px-6 py-3.5 text-sm font-black text-fuchsia-50 shadow-[0_14px_34px_rgba(168,85,247,0.14)] transition hover:-translate-y-0.5 hover:border-fuchsia-200/45 hover:from-fuchsia-500/28 hover:to-violet-500/28'><SparklesIcon className='h-4 w-4' />AI创作</FeiXiangSmartLink>
...
const FALLBACK_INVITE_REWARD_CONFIG: InviteRewardConfig = {
  enabled: true,
  new_user_quota: 0,
  inviter_reward: 0,
  invitee_reward: 0,
  new_user_quota_display: '以当前平台配置为准',
  inviter_reward_display: '以当前平台配置为准',
  invitee_reward_display: '以当前平台配置为准',
  unit_label: '平台额度',
}

function normalizeInviteRewardConfig(payload: any): InviteRewardConfig {
  const data = payload?.data && typeof payload.data === 'object' ? payload.data : payload
  return {
    ...FALLBACK_INVITE_REWARD_CONFIG,
    enabled: Boolean(data?.enabled ?? true),
    new_user_quota: Number(data?.new_user_quota ?? 0),
    inviter_reward: Number(data?.inviter_reward ?? 0),
    invitee_reward: Number(data?.invitee_reward ?? 0),
    new_user_quota_display: String(data?.new_user_quota_display || FALLBACK_INVITE_REWARD_CONFIG.new_user_quota_display),
    inviter_reward_display: String(data?.inviter_reward_display || FALLBACK_INVITE_REWARD_CONFIG.inviter_reward_display),
    invitee_reward_display: String(data?.invitee_reward_display || FALLBACK_INVITE_REWARD_CONFIG.invitee_reward_display),
    unit_label: String(data?.unit_label || '平台额度'),
    source: data?.source ? String(data.source) : 'quota_settings',
  }
}

function resolveAffCode(payload: any): string {
  const data = payload?.data ?? payload?.aff_code ?? payload?.affCode ?? payload?.code
  if (typeof data === 'string') return data.trim()
  if (data && typeof data === 'object') {
    return String(data.aff_code || data.affCode || data.code || data.aff || '').trim()
  }
  return ''
}
...
function InviteRewardCard({
  ...
  const toneClass =
    tone === 'cyan'
      ? 'border-cyan-300/18 bg-cyan-400/[0.07] shadow-[0_18px_70px_rgba(34,211,238,0.10)]'
      : tone === 'purple'
        ? 'border-violet-300/18 bg-violet-400/[0.07] shadow-[0_18px_70px_rgba(139,92,246,0.10)]'
        : 'border-pink-300/18 bg-pink-400/[0.07] shadow-[0_18px_70px_rgba(236,72,153,0.10)]'

  return (
    <div className={cx('rounded-[2rem] border p-7 backdrop-blur-xl', toneClass)}>
      <div className='flex h-12 w-12 items-center justify-center rounded-2xl border border-white/12 bg-white/[0.06] text-white'>
        {icon}
      </div>
...
function InviteStepCard({
  ...
  return (
    <div className='relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 backdrop-blur-xl'>
      <div className='absolute -right-8 -top-8 h-28 w-28 rounded-full bg-gradient-to-br from-cyan-400/16 to-violet-500/10 blur-2xl' />
      <div className='relative flex items-center gap-4'>
        ...
        <div className='text-sm font-black uppercase tracking-[0.28em] text-cyan-100/55'>{index}</div>
      </div>
      <h3 className='relative mt-5 text-lg font-black text-white'>{title}</h3>
      <p className='relative mt-3 text-sm leading-7 text-white/52'>{desc}</p>
    </div>
  )
}

function InviteRuleItem({ children }: { children: ReactNode }) {
  return (
    <li className='flex gap-3 text-sm leading-7 text-white/62'>
...
export function FeiXiangInviteRewardsPage() {
  const [config, setConfig] = useState<InviteRewardConfig>(FALLBACK_INVITE_REWARD_CONFIG)
  ...
  useEffect(() => {
    ...
    async function loadInviteConfig() {
      try {
        const response = await fetch('/api/invite/reward-config', {
          headers: { Accept: 'application/json' },
        })
        if (!response.ok) throw new Error(`reward_config_${response.status}`)
        const payload = await response.json()
        if (!cancelled) {
          setConfig(normalizeInviteRewardConfig(payload))
          setConfigError(false)
        }
      } catch {
        if (!cancelled) {
          setConfig(FALLBACK_INVITE_REWARD_CONFIG)
          setConfigError(true)
        }
      }
    }
    ...
    async function loadAffCode() {
      try {
        ...
        if (!response.ok) throw new Error(`aff_${response.status}`)
        const payload = await response.json()
        if (!cancelled) setAffCode(resolveAffCode(payload))
      } catch {
        if (!cancelled) setAffCode('')
      } finally {
        if (!cancelled) setAffLoaded(true)
      }
    }
  ...
  const inviteLink = affCode ? `${BASE_URL}/register?aff=${encodeURIComponent(affCode)}` : ''
  const inviteText = inviteLink
    ? `我在使用 FeiXiangApi 调用 GPT、Claude、Gemini 和视频生成模型。通过我的邀请链接注册可获得新人奖励：${inviteLink}`
    : ''

  return (
    <PageShell>
      <FeiXiangNavActiveRouteFix />
      <section className='relative overflow-hidden px-4 py-20 sm:px-6 lg:px-8'>
        <div className='absolute inset-0 -z-10 bg-[radial-gradient(circle_at_20%_15%,rgba(34,211,238,0.14),transparent_32%),radial-gradient(circle_at_80%_5%,rgba(139,92,246,0.18),transparent_35%),radial-gradient(circle_at_50%_75%,rgba(236,72,153,0.10),transparent_30%)]' />
        <div className='mx-auto max-w-7xl'>
          <div className='mx-auto max-w-4xl text-center'>
            <Badge><GiftIcon className='h-4 w-4' /> 老带新奖励计划</Badge>
            <h1 className='mt-7 text-5xl font-black tracking-tight sm:text-6xl lg:text-7xl'>
              邀请好友加入 <GradientText>FeiXiangApi</GradientText>
            </h1>
            <p className='mt-7 text-xl font-black tracking-[0.28em] text-white sm:text-3xl'>
              双方都得 API 额度奖励
            </p>
            <p className='mx-auto mt-6 max-w-3xl text-base leading-8 text-white/58 sm:text-lg'>
              分享你的专属邀请链接，好友通过链接完成注册后，你和好友都可以获得平台额度奖励，可用于调用 GPT、Claude、Gemini、视频生成等模型服务。
            </p>
            <div className='mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row'>
              {inviteLink ? (
                <CopyButton
                  ...
                  label='复制我的邀请链接'
                  copiedLabel='已复制邀请链接'
                  className='border-0 bg-gradient-to-r from-cyan-400 to-violet-500 px-7 py-3.5 text-white shadow-[0_16px_42px_rgba(34,211,238,0.22)] hover:from-cyan-300 hover:to-violet-400'
                />
              ) : (
                <FeiXiangSmartLink
                  href='/sign-in'
                  className='inline-flex min-w-52 items-center justify-center gap-2 rounded-full bg-gradient-to-r from-cyan-400 to-violet-500 px-7 py-3.5 text-sm font-black text-white shadow-[0_16px_42px_rgba(34,211,238,0.22)] transition hover:-translate-y-0.5 hover:from-cyan-300 hover:to-violet-400'
                >
              ...
              <a
                href='#reward-rules'
                className='inline-flex min-w-44 items-center justify-center gap-2 rounded-full border border-white/14 bg-white/[0.035] px-7 py-3.5 text-sm font-black text-white/85 transition hover:border-white/24 hover:bg-white/[0.065]'
              >
                查看奖励规则
              </a>
            </div>
            {configError ? (
              <p className='mt-5 text-sm text-amber-200/80'>奖励配置接口暂时不可用，当前页面以平台后台配置为准。</p>
            ) : null}
          </div>

          <div className='mt-14 grid gap-4 md:grid-cols-3'>
            {[
              ['双方都有奖励', '邀请人与受邀人均可获得平台额度。'],
              ['自动发放额度', '好友完成注册后，系统自动处理奖励。'],
              ['团队好友共享', '适合团队成员、朋友和开发者社群传播。'],
            ].map(([title, desc]) => (
              <div key={title} className='rounded-3xl border border-white/10 bg-white/[0.035] p-5 text-center backdrop-blur-xl'>
                <strong className='text-white'>{title}</strong>
                <p className='mt-2 text-sm text-white/48'>{desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className='mx-auto max-w-7xl px-4 pb-10 sm:px-6 lg:px-8'>
        <div className='grid gap-5 lg:grid-cols-3'>
          <InviteRewardCard
            ...
            title='邀请人奖励'
            value={config.inviter_reward_display}
            desc='好友通过你的邀请链接注册成功后，你将获得平台额度奖励。'
            tone='cyan'
          />
          <InviteRewardCard
            icon={<GiftIcon className='h-5 w-5' />}
            title='受邀人奖励'
            value={config.invitee_reward_display}
            desc='新用户通过邀请链接注册后，将获得额外受邀奖励。'
            tone='purple'
          />
          <InviteRewardCard
            icon={<SparklesIcon className='h-5 w-5' />}
            title='新人初始额度'
            value={config.new_user_quota_display}
            desc='新用户注册后，可获得平台初始体验额度。'
            tone='pink'
          />
        </div>
      </section>
      ...
      <section className='mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8'>
        <div className='mb-8 text-center'>
          ...
          <h2 className='mt-5 text-4xl font-black text-white'>三步完成邀请</h2>
          <p className='mx-auto mt-4 max-w-2xl text-white/55'>复制链接、分享好友、双方获得奖励，全流程自动完成。</p>
        </div>
        <div className='grid gap-5 md:grid-cols-3'>
          <InviteStepCard index='STEP 01' title='复制专属邀请链接' desc='登录后系统自动生成你的专属邀请链接。' icon={<CopyIcon className='h-5 w-5' />} />
          <InviteStepCard index='STEP 02' title='分享给好友注册' desc='好友通过链接进入注册页面并完成注册。' icon={<UsersIcon className='h-5 w-5' />} />
          <InviteStepCard index='STEP 03' title='双方获得奖励' desc='系统自动发放邀请奖励和受邀奖励。' icon={<GiftIcon className='h-5 w-5' />} />
        </div>
      </section>

      <section className='mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8'>
        <div className='grid gap-6 rounded-[2.5rem] border border-white/10 bg-white/[0.04] p-6 shadow-[0_24px_100px_rgba(139,92,246,0.12)] backdrop-blur-xl lg:grid-cols-[1.1fr_1.4fr] lg:p-8'>
          <div>
            <Badge>My Invite Link</Badge>
            <h2 className='mt-5 text-3xl font-black text-white'>我的邀请链接</h2>
            <p className='mt-4 text-sm leading-7 text-white/55'>
              {inviteLink
                ? `成功邀请好友注册，你将获得 ${config.inviter_reward_display} ${config.unit_label}，好友也将获得 ${config.invitee_reward_display} 受邀奖励。`
                : affLoaded
                  ? '登录或注册后，你可以复制邀请链接并分享给好友。'
                  : '正在检查登录状态和邀请链接...'}
            </p>
          </div>
          <div className='rounded-[2rem] border border-white/10 bg-[#08081a]/80 p-5'>
            {inviteLink ? (
              <>
                <div className='rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm text-white/80 break-all'>{inviteLink}</div>
                <div className='mt-4 grid gap-3 sm:grid-cols-2'>
                  <CopyButton value={inviteLink} label='复制链接' copiedLabel='已复制链接' className='border-cyan-300/25 bg-cyan-300/10 text-cyan-50 hover:border-cyan-200/50' />
                  <CopyButton value={inviteText} label='复制邀请文案' copiedLabel='已复制文案' className='border-violet-300/25 bg-violet-300/10 text-violet-50 hover:border-violet-200/50' />
                </div>
              </>
            ) : (
              <div className='grid gap-3 sm:grid-cols-2'>
                <FeiXiangSmartLink href='/sign-in' className='rounded-xl border border-white/14 px-5 py-3 text-center text-sm font-black text-white/85 transition hover:border-white/24 hover:bg-white/[0.06]'>登录</FeiXiangSmartLink>
                <FeiXiangSmartLink href='/register' className='rounded-xl bg-gradient-to-r from-pink-500 to-orange-400 px-5 py-3 text-center text-sm font-black text-white shadow-[0_14px_34px_rgba(244,63,94,0.22)]'>立即注册</FeiXiangSmartLink>
              </div>
            )}
          </div>
        </div>
      ...
      <section id='reward-rules' className='mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8'>
        <div className='grid gap-6 lg:grid-cols-[1fr_1fr]'>
          <div className='rounded-[2rem] border border-white/10 bg-white/[0.04] p-7 backdrop-blur-xl'>
            <h2 className='text-2xl font-black text-white'>奖励规则</h2>
            <ul className='mt-6 grid gap-3'>
              <InviteRuleItem>好友通过你的专属邀请链接注册后，邀请人可获得 {config.inviter_reward_display} {config.unit_label}。</InviteRuleItem>
              <InviteRuleItem>好友作为受邀用户，可额外获得 {config.invitee_reward_display} {config.unit_label}。</InviteRuleItem>
              <InviteRuleItem>新用户注册后，默认可获得 {config.new_user_quota_display} 初始额度。</InviteRuleItem>
              <InviteRuleItem>奖励会发放到平台额度账户，不支持折现或提现。</InviteRuleItem>
              <InviteRuleItem>异常注册、批量注册、恶意刷奖励行为可能不会计入奖励。</InviteRuleItem>
              <InviteRuleItem>实际奖励额度以后端当前配置为准，页面刷新后会自动同步。</InviteRuleItem>
            </ul>
          </div>

          <div className='rounded-[2rem] border border-white/10 bg-white/[0.04] p-7 backdrop-blur-xl'>
            <h2 className='text-2xl font-black text-white'>常见问题</h2>
            <div className='mt-6 grid gap-4'>
              {[
                ['邀请奖励多久到账？', '通常在好友完成注册后自动发放。'],
                ['在哪里查看奖励？', '可以在控制台的钱包、额度记录或邀请记录中查看。'],
                ['可以邀请多少人？', '正常邀请不限制，但异常注册会触发风控。'],
                ['好友没有收到奖励怎么办？', '确认好友是否通过你的专属邀请链接完成注册。'],
              ].map(([question, answer]) => (
                <div key={question} className='rounded-2xl border border-white/10 bg-white/[0.035] p-4'>
      ...
      <section className='mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8'>
        <div className='relative overflow-hidden rounded-[2.5rem] border border-cyan-200/14 bg-gradient-to-br from-cyan-400/[0.10] via-violet-500/[0.08] to-pink-500/[0.08] p-8 text-center shadow-[0_24px_100px_rgba(34,211,238,0.12)]'>
          <div className='absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(255,255,255,0.10),transparent_40%)]' />
          <div className='relative'>
            <h2 className='text-3xl font-black text-white'>开始邀请好友，领取更多 API 调用额度</h2>
            <p className='mx-auto mt-3 max-w-2xl text-sm leading-7 text-white/55'>奖励数值实时读取后台额度设置，后台修改后前台刷新即可同步。</p>
            <div className='mt-7 flex flex-col items-center justify-center gap-3 sm:flex-row'>
              {inviteLink ? (
                <CopyButton value={inviteLink} label='复制我的邀请链接' copiedLabel='已复制邀请链接' className='border-0 bg-gradient-to-r from-cyan-400 to-violet-500 px-7 py-3 text-white' />
              ) : (
                <FeiXiangSmartLink href='/sign-in' className='rounded-full bg-gradient-to-r from-cyan-400 to-violet-500 px-7 py-3 text-sm font-black text-white'>登录后获取邀请链接</FeiXiangSmartLink>
              )}
              <FeiXiangSmartLink href='/dashboard' className='rounded-full border border-white/14 bg-white/[0.035] px-7 py-3 text-sm font-black text-white/85 hover:bg-white/[0.06]'>进入控制台</FeiXiangSmartLink>
            </div>
          </div>
        </div>
      </section>
