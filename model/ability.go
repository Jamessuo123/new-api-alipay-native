...
func GetChannel(group string, model string, retry int, requestPath string) (*Channel, error) {
	...
	if len(abilities) == 0 {
		if fallbackAbilities, fallbackErr := feiXiangVideoModelChannelFallback(group, model); fallbackErr == nil && len(fallbackAbilities) > 0 {
			abilities = fallbackAbilities
		} else if fallbackErr != nil {
			common.SysLog(fmt.Sprintf("feiXiangVideoModelChannelFallback failed: %v", fallbackErr))
		}
	}
	channel := Channel{}
	if len(abilities) > 0 {
...
func feiXiangVideoModelChannelFallback(group string, modelName string) ([]Ability, error) {
	modelName = strings.TrimSpace(modelName)
	if !feiXiangAbilityIsSeedanceModelName(modelName) {
		return nil, nil
	}
	...
	for _, channel := range channels {
		if channel == nil || !feiXiangAbilityIsSeedanceVideoChannel(channel) {
			continue
		}
		if !feiXiangAbilityCSVContains(channel.Group, group) {
			continue
		}
		if !feiXiangAbilityCSVContains(channel.Models, modelName) {
			continue
		}
		key := fmt.Sprintf("%s|%s|%d", group, modelName, channel.Id)
		if _, ok := seen[key]; ok {
			continue
		}
		seen[key] = struct{}{}
...
func feiXiangAbilityIsSeedanceVideoChannel(channel *Channel) bool {
	if channel == nil {
		return false
	}
...
func feiXiangAbilityIsSeedanceModelName(modelName string) bool {
	modelName = strings.ToLower(strings.TrimSpace(modelName))
	return strings.Contains(modelName, "seedance") || strings.Contains(modelName, "doubao-seedance")
}

func feiXiangAbilityCSVContains(csvValue string, target string) bool {
	target = strings.TrimSpace(target)
	if target == "" {
		return false
	}
	for _, part := range strings.Split(csvValue, ",") {
